...
if "document_type" not in st.session_state:
    st.session_state.document_type = "auto"
...
def display_results(results: dict):
    ...
    with col2:
        # CSV download option
        if st.button("📥 Request CSV Format", use_container_width=True):
            # Re-process with CSV format
            if st.session_state.uploaded_file:
                ...
                try:
                    csv_results = client.upload_and_process(
                        file=file_obj,
                    ...
                    if "csv_data" in csv_results:
                        create_download_button(csv_results, "ocr_results", "csv")
                except Exception as e:
